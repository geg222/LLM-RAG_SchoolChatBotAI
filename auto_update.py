#!/usr/bin/env python3
"""
한성대학교 챗봇 자동 업데이트 스크립트
crawl.py -> ocrmac.py -> upload.py 순서로 실행
"""
...
def main():
    """메인 파이프라인 실행 함수"""
    print("한성대학교 챗봇 자동 업데이트 시작")
    print(f"실행 날짜: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # 실행할 스크립트 목록
    scripts = [
        ("crawl.py", "학사 공지사항 크롤링"),
        ("ocrmac.py", "이미지 OCR 처리"),
        ("upload.py", "벡터 데이터베이스 업로드")
    ]
